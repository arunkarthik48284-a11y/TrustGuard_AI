...
const MetricCard = ({ title, value, change, isPositive = true, icon: Icon, color = 'emerald', subtitle }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900/70 backdrop-blur-md border border-slate-200 dark:border-slate-800 p-6 rounded-2xl relative overflow-hidden transition-all duration-200 hover:border-slate-300 dark:hover:border-slate-700 space-y-4 shadow-sm">
      ...
      <div className="flex items-center justify-between text-xs pt-1 border-t border-slate-200 dark:border-slate-800/80">
        {change !== undefined && (
          <div className={`flex items-center gap-1.5 font-semibold ${isPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
            {isPositive ? <TrendingUp className="w-3.5 h-3.5" strokeWidth={1.5} /> : <TrendingDown className="w-3.5 h-3.5" strokeWidth={1.5} />}
            <span>{change}</span>
          </div>
        )}
        {subtitle && <span className="text-slate-500 dark:text-slate-400 text-[11px] font-medium">{subtitle}</span>}
      </div>
    </div>
  );
};
