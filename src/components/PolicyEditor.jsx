...
import { Check, Save, ToggleLeft, ToggleRight } from 'lucide-react';
...
const PolicyEditor = () => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900/70 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-slate-800 p-6 space-y-6 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          <div className="flex items-center gap-3">
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">{policyName}</h3>
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/15 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 uppercase">
              {isActive ? 'ACTIVE' : 'INACTIVE'}
            </span>
          </div>
          <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Configure global AI firewall boundaries and maximum allowed risk thresholds.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="text-lg font-extrabold text-emerald-700 dark:text-emerald-400">{rules.max_allowed_risk_score} / 100</span>
          </div>
          <input
            type="range"
            min="20"
            ...
            className="w-full h-2 bg-slate-200 dark:bg-slate-900 rounded-lg appearance-none cursor-pointer accent-emerald-500"
          />
          <p className="text-[11px] text-slate-500 dark:text-slate-400">
            Payloads exceeding risk score <span className="text-slate-900 dark:text-slate-200 font-semibold">{rules.max_allowed_risk_score}</span> will be blocked.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-3 pt-2">
        <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Active Guardrail Rules</h4>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            >
              <div>
                <h5 className="text-xs font-bold text-slate-900 dark:text-slate-100">{rule.label}</h5>
                <p className="text-[11px] text-slate-600 dark:text-slate-400 mt-1 leading-relaxed">{rule.desc}</p>
              </div>
              {rules[rule.key] ? (
                <ToggleRight className="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0" strokeWidth={1.5} />
              ) : (
                <ToggleLeft className="w-5 h-5 text-slate-400 dark:text-slate-600 shrink-0" strokeWidth={1.5} />
              )}
            </div>
          ))}
        </div>
