  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 pb-12">
      ...
      <main className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6">
        <div>
          <h2 className="text-xl font-extrabold text-slate-100">Organization Settings</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="bg-slate-900/70 backdrop-blur-md p-6 rounded-2xl border border-slate-800 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-slate-100">TrustGuard SDK API Key</h3>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 block">Production Key</label>
              <div className="flex items-center gap-2">
                <input
                  type={showKey ? 'text' : 'password'}
                  readOnly
                  value={apiKey}
                  className="w-full bg-slate-950 text-emerald-400 font-mono text-xs p-3 rounded-xl border border-slate-800 focus:outline-none min-h-[44px]"
                />
                <button
                  onClick={() => setShowKey(!showKey)}
                  className="p-3 rounded-xl bg-slate-950 text-slate-400 hover:text-slate-200 border border-slate-800 min-h-[44px] flex items-center justify-center shrink-0"
                >
                  {showKey ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                </button>
                <button
                  onClick={copyKey}
                  className="p-3 rounded-xl bg-slate-950 text-emerald-400 hover:text-emerald-300 border border-slate-800 min-h-[44px] flex items-center justify-center shrink-0"
                >
            ...
            <div className="pt-2 flex items-center justify-between">
              <button
                ...
                className="px-3.5 py-2 rounded-xl bg-slate-950 hover:bg-rose-950 text-rose-400 border border-slate-800 text-xs font-semibold flex items-center gap-1.5 transition-colors min-h-[40px]"
              >
          ...
          <div className="bg-slate-900/70 backdrop-blur-md p-6 rounded-2xl border border-slate-800 space-y-4">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-bold text-slate-100">Real-Time Incident Webhooks</h3>
            </div>
            <p className="text-xs text-slate-400">
            ...
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-300 block">Webhook Endpoint URL</label>
              <input
                type="url"
                value={webhookUrl}
                onChange={(e) => setWebhookUrl(e.target.value)}
                placeholder="https://api.yourdomain.com/webhooks/security"
                className="w-full bg-slate-950 text-slate-200 font-mono text-xs p-3 rounded-xl border border-slate-800 focus:outline-none focus:border-indigo-500/50 min-h-[44px]"
              />
            </div>
